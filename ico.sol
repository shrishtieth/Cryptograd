...
contract CryptoGradICO is Ownable, ReentrancyGuard {
	...
	function updateUnlockPrice(uint256 _price) external onlyOwner{
		unlockPrice = _price;
	}
	...
	function buyToken(address token, uint256 amount, address referrer) external payable   {
		...
		uint256 tokenAmount = (amount * _currencyPrice) / tokenPrice;
			...
			amountRaised += (amount * _currencyPrice)/ 10 ** 8;
	...
	function distributeRevenueEth(uint256 amount, address user, uint256 _tokenPrice) public {
	...
	function claimReward() external {
        if(userRewardBalance[msg.sender][weth] > 0){
			...
			emit RewardClaimed(msg.sender, weth, userRewardBalance[msg.sender][weth]);
		...
		if(userRewardBalance[msg.sender][usdc] > 0){
			...
			emit RewardClaimed(msg.sender, usdc, userRewardBalance[msg.sender][usdc]);
		...
		if(userRewardBalance[msg.sender][usdt] > 0){
			...
			emit RewardClaimed(msg.sender, usdt, userRewardBalance[msg.sender][usdt]);
